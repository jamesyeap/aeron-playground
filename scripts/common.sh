...
LOG4J_1_2_API_JAR="$(find "$GRADLE_CACHE/org.apache.logging.log4j/log4j-1.2-api/2.23.1" -name 'log4j-1.2-api-2.23.1.jar' -print -quit 2>/dev/null)"
LOG4J_OVER_SLF4J_JAR="$(find "$GRADLE_CACHE/org.slf4j/log4j-over-slf4j/2.0.18" -name 'log4j-over-slf4j-2.0.18.jar' -print -quit 2>/dev/null)"
SLF4J_API_JAR="$(find "$GRADLE_CACHE/org.slf4j/slf4j-api/2.0.13" -name 'slf4j-api-2.0.13.jar' -print -quit 2>/dev/null)"

LOGGING_JARS="${SLF4J_API_JAR}:${LOG4J_API_JAR}:${LOG4J_CORE_JAR}:${LOG4J_SLF4J_JAR}:${LOG4J_1_2_API_JAR}:${LOG4J_OVER_SLF4J_JAR}"
